...
namespace WebApi.Utility
{
    public class HttpClient
    {
        ...
        private Dictionary<string, string> postingData = new Dictionary<string, string>();
        private Dictionary<string, string> getMethodParam = new Dictionary<string, string>();
        private string url;
        ...
        {
            get { return _multipart; }
            set { _multipart = value; }
        }
        /// <summary>
        /// 期望的回应的语言
        /// </summary>
        ...
        {
            set { postingData = value; }
            get { return postingData; }
        }

        public Dictionary<string, string> GetMethodParam
        {
            set { getMethodParam = value; }
            get { return getMethodParam; }
        }
        ...
        public void SetMultiPart(bool fg)
        {
            _multipart = fg;
        }

        public void SetGetMethodUrl()
        {
            if (verb == HttpVerb.GET)
            {
                if (getMethodParam.Count > 0)
                {
                    StringBuilder sb = new StringBuilder();
                    foreach (string key in getMethodParam.Keys)
                    {
                        sb.AppendFormat("{0}={1}&", Uri.EscapeDataString(key), Uri.EscapeDataString(getMethodParam[key]));
                    }
        ...
        private HttpWebRequest CreateRequest()
        {
            ...
            {
                req.Headers.Add("accessToken", AccessToken);
            }
            ...
            if (verb == HttpVerb.POST)
            {
                ...
                else
                {
                    ...
                    foreach (string key in postingData.Keys)
                    {
                        sb.AppendFormat("{0}={1}&", Uri.EscapeDataString(key), Uri.EscapeDataString(postingData[key]));
                    }
                    if (sb.Length > 0)
                        sb.Length--;
        ...
        public bool GetPdf(string sFileFullName)
        {
            bool fg = true;
            try
            {
                ...
                FileInfo fif = new FileInfo(sFileFullName);
                if (!fif.Directory.Exists)
                {
                    fif.Directory.Create();
                }

                using (FileStream MyFileStream = new FileStream(sFileFullName, FileMode.Create, FileAccess.Write))
                {
